"""Authenticated diagnostic rendering and export helpers with no raw secret-bearing state."""
...
class DiagnosticFilters:
    ...
    def query_string(self) -> str:
        ...
        if self.hours is not None:
            pairs.append(("hours", str(self.hours)))
  ...
  <form method="get" action="/admin/diagnostics">
    <label>Severity
      <select name="severity">
        {_severity_options(severity_value)}
      </select>
    </label>
    <label>Source
...
def build_diagnostic_bundle(
    *,
    ...
    status = _safe_status_document(last_cycle)
    runtime = {
    ...
    with zipfile.ZipFile(buffer, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        _writestr(archive, "status.json", _json_text(status))
...
def _severity_options(selected: str) -> str:
    options = [("", "All"), ("INFO", "INFO"), ("WARNING", "WARNING"), ("ERROR", "ERROR")]
    return "".join(_option(value, label, selected) for value, label in options)


def _hours_options(selected: str) -> str:
    options = [("1", "1 hour"), ("6", "6 hours"), ("24", "24 hours"), ("168", "7 days"), ("720", "30 days"), ("all", "All retained")]
    return "".join(_option(value, label, selected) for value, label in options)
